...
const birthPerson = personArray.filter(
  person => new Date(person.birthDate) < new Date("Jan 1, 1990")
);

console.log("7.", birthPerson);
...
let oldies = personArray.filter(function birthday(birthYear) {
  ...
  if (stringElements.match(r) <= 1990) {
    console.log("8. ", birthYear.firstName + " " + birthYear.lastName);
  }
});
